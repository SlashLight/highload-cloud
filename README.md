...
Все сервисы кроме nginx, и URL verifier стоит расположить в один kubernetes. В сумме нужно 235 ядер, 24 ГБ RAM и сетевая карта минимум на 59 Гбит/с.
CDN сервера будут состоять из nginx и сервиса URL verifier, который может представлять собой LUA скрипт для самого nginx. Основной балансировщик, в роли которого выступает nginx нужно разместить в отдельной куберноде, так как ему требуется значительная пропускная способность.

| Название   | Хостинг | Конфигурация                                | Cores | Cnt  | Покупка |
| ---------- | ------- | ------------------------------------------- | ----- | ---- | ------- | 
| kubernode  | own     | 1x7713P/2x128GB/2xNVMe1T/1x100Gb/s          |    64 |  5   |  €10667 |
| nginx node | own     | 1x7713P/2x128GB/1xNVMe1T/1x100Gb/s          |    64 |  4   |  €10296 |
| CDN node   | own     | 1x7713P/2x128GB/1xNVMe1T/1x100Gb/s          |    64 |  18  |  €10296 |
...
| ClickHouse | own/bare metal     | 1x7543P/2x16GB/1xNVMe800G/1x10Gb/s          |    32 |  2   |  €6665   |
| Redis      | own/bare metal     | 1x7453P/2x128GB/2xNVMe1T/2x100Gb/s          |    32 |  2   |  €6033  |
