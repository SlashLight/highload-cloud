...
FILE_DATA ||--}| CHUNK : file_parts
CHUNK o{--}| GROUP_OSD : file_parts_in_groups
FILE_DATA |{--|| FILE_META : contains
FILE_DATA o{--|| POOL : files_in_pool
POOL |{--|| CRUSH_RULE : rule_for_managing_files_in_pools
```

Название таблицы     |     Описание таблицы
---------------------|----------------------
FILE_DATA            | Таблица связывает объекты с их физическим расположением. По                           pool_id можно узнать номер пула, после чего с помощью хэш-                            фукнции можно узнать номер группы, а по номеру группы и номер                         диска.
CHUNK                | Таблица хранит информацию о том, в каких группах хранятся                             части файла
POOL                 | Таблица содержит информацию об имени пула, факторе репликации                         (по умолчанию 3) и ссылке на правило для алгоритма CRUSH.
CRUSH_RULE           | Таблица содержит информацию о правилах для алгоритма CRUSH. 
                       Здесь содержится информация о числе групп, участвующих в                              размещении, а также об уровне отказоустойчивости (host, rack,                         datacenter)
GROUP_OSD            | Таблица связывает группу с теми OSD, которые привязаны к этой                         группе
...
-----------------------|------------------------
CRUSH                  | Алгоритм распределения файлов по группам
Presigned URL          | Алгоритм создания URL для доступа к файлам

### Алгоритм CRUSH
Вначале файлы разбиваются на чанки по 4 Мб каждый. Далее вычисляется хеш каждого чанка и с его помощью вычсиляется группа, в которую попадет чанк. По умолчанию за каждой группой закреплены 3 диска, один из которых является основным и запись в него делается напрямую клиентом, а два других являются резервными и репликация на них происходит по приватной сети.
CRUSH можно настраивать таким образом, что реплики будут находиться на разных хостах, стойках или в разных датацентрах. 
Когда в кластер добавляется новый хост или диск, CRUSH запускает операцию повторной балансировки, в ходе которой он перемещает данные с сущесвтующих хостов/дисков на новые хосты/диски.

### Алгоритм генерации presigned URL
...
Теперь пользователь может скачать объект напрямую их хранилища, либо загрузить его напрямую в хранилище
