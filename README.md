...
erDiagram
    ...
    FILE_USER_LOOKUP{
        ...
        UUID user_id
        TIMESTAMP created_at
        PRIMARY KEY ((file_hash), file_uuid)
    }
```
...
| Kafka                      | Репликация (multi-broker)          | Partitioning, ISR (In-Sync Replicas), automatic leader election, at most once | Гарантированная доставка событий. Обеспечение отказоустойчивости через кластер брокеров и репликацию партиций |
