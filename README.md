...
- **Загрузка файлов на облако** - 3 запроса/день * (47 Мб + 119 байт) = 340 Мб/день; (!)
...
При запросе метаинформации о файле мы запрашиваем его название (31 байт), его формат (7 байт), его размер (8 байт) и дату создания (8 байт). Итого 54 байта. Пиковый показатель 4.6млн * (17 запросов/день / 86400) * (54 байта) * 8 = 382 Кбит/с. За сутки 4.6млн * 17 * 54 байта = 4 ГБ/сутки.
...
Отображение информации о файле | 382 * 2 ^ (-20) | 4
Итого | 333 | 3589303
...
Примем во внимние, что облачное хранилище не является сервисом, для которого критически важна задержка. Также все запросы будут использовать протокол HTTP вместе с SSL. Поэтому стоит выбрать L7 балансировку трафика внутри датацентра.
